/**
 * @keeb/alpine/install model — install Alpine Linux to disk on a
 * PXE-booted VM using setup-alpine with a generated answer file, followed
 * by a chroot-based post-install (SSH keys, apk repos, qemu-guest-agent).
 */
...
const GlobalArgs = z.object({
  sshHost: z.string().describe(
    "SSH hostname/IP of the PXE-booted VM (set via CEL from testVm)",
  ),
...
const InstallArgs = z.object({
  ...
  disk: z.string().default("/dev/sda").describe(
    "Target disk for Alpine install (default '/dev/sda')",
  ),
});
...
/** Swamp model definition for `@keeb/alpine/install`. */
export const model: {
  type: string;
  version: string;
  ...
  methods: {
    install: {
      ...
      execute: async (args, context) => {
        ...
        console.log(
          `[install] Installing Alpine to ${disk} on ${sshHost} (hostname: ${hostname})`,
        );
        ...
        console.log(
          `[install] Loading SCSI modules and waiting for disk ${disk}...`,
        );
        await sshExec(
          sshHost,
          sshUser,
          `modprobe virtio_scsi 2>/dev/null; modprobe sd_mod 2>/dev/null; mdev -s 2>/dev/null; for i in $(seq 1 30); do [ -b ${disk} ] && exit 0; echo "Waiting for ${disk} ($i/30)..."; sleep 2; done; echo "${disk} not found after 60s"; exit 1`,
        );
        ...
        await sshExec(
          sshHost,
          sshUser,
          `cat > /tmp/setup-alpine.conf << 'ANSWEREOF'\n${answerFile}ANSWEREOF`,
        );
        console.log(`[install] Answer file written`);

        // Step 3: Run setup-alpine
        console.log(
          `[install] Step 3: Running setup-alpine (this may take a while)...`,
        );
        await sshExec(
          sshHost,
          sshUser,
          `ERASE_DISKS=${disk} setup-alpine -e -f /tmp/setup-alpine.conf`,
        );
        console.log(`[install] Alpine installed to ${disk}`);

        // Step 4: Post-install chroot setup
        console.log(
          `[install] Step 4: Post-install setup (SSH keys, guest agent, repos)...`,
        );
